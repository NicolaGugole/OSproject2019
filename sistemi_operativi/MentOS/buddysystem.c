...
page_t *bb_alloc_pages(zone_t *zone, unsigned int order)
{
	...
	for (current_order = order; current_order < MAX_ORDER; ++current_order) {
		...
		area = &(zone->free_area[current_order]); //ATTENZIONE ALLA FRECCIA
...
block_found:
	...
	list_head_del(&(page->lru)); // prova a scrivere &(area->free_list)
...
void bb_free_pages(zone_t *zone, page_t *page, unsigned int order)
{
	...
	while (order < MAX_ORDER - 1) {
		...
		// page in the list of free blocks.
 		dbg_print("\nTrying buddy private: %d order: %d\n", buddy->private, order);
